#!/usr/bin/env python3
"""Independent verifier for certified conditional MSCP envelope campaigns.
...
def parse_graph(path: Path) -> Graph:
    ...
    # Some DIMACS archives declare twice the number of undirected edge records.
    # Accept exactly m or 2m while still requiring one unique undirected record per line.
    if len(edges) != edge_records or declared not in {edge_records, 2 * edge_records}:
